...
describe('MDM command policy validation', () => {
  it('allows IT admin to request a sensitive command only with sufficient reason', () => {
    const accepted = validateMdmCommandRequest(
      {
        ...
        requestedBy: 'it-admin-001',
        requestedByRole: 'it_admin',
        reason: 'Managed device security incident requires temporary lock',
      },
      eligibleDevice,
    );
    ...
    expect(accepted.status).toBe('accepted_for_queue');
  ...
  it('keeps non-eligible devices limited to diagnostics in the IT console', () => {
    const controls = getMdmConsoleControls({
      ...eligibleDevice,
    ...
    expect(controls.find((item) => item.commandType === 'uninstall_app')?.enabled).toBe(false);
  });
});
