...
export function DesktopCloudApprovalPanel({ language }: { language: Language }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} style={sx({ position: "fixed", right: 24, bottom: 24, zIndex: 45, border: "1px solid #bfd3ec", borderRadius: 999, padding: "12px 16px", background: "#0b6ff6", color: "white", fontWeight: 900, boxShadow: "0 14px 34px rgba(15,63,126,.2)" })}>☁ {th ? "Cloud อนุมัติ" : "Cloud approvals"}{pending.length ? ` (${pending.length})` : ""}</button>
    {open ? <div style={sx({ position: "fixed", inset: 0, zIndex: 80, background: "rgba(10,22,40,.55)", display: "grid", placeItems: "center", padding: 24 })} onMouseDown={() => setOpen(false)}>
      <section style={sx({ width: "min(1180px, 96vw)", maxHeight: "88vh", overflow: "auto", background: "#fff", borderRadius: 22, boxShadow: "0 24px 80px rgba(0,0,0,.28)", padding: 24 })} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div style={sx({ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 })}>
          {(data?.plans ?? []).map((plan) => <div key={plan.code} style={sx({ border: "1px solid #d9e5f3", borderRadius: 16, padding: 14, background: "#f9fcff" })}><b>{plan.label_th}</b><small style={sx({ display: "block", color: "#60758e", margin: "4px 0 8px" })}>{plan.code}</small><input value={prices[plan.code] ?? ""} placeholder="ราคา (บาท)" inputMode="decimal" onChange={(event) => setPrices((current) => ({ ...current, [plan.code]: event.target.value.replace(/[^0-9.]/g, "") }))} style={sx({ width: "100%", height: 40, border: "1px solid #cbd9e8", borderRadius: 10, padding: "0 10px" })} /><button disabled={busy} onClick={() => void savePlan(plan)} style={sx({ marginTop: 10, width: "100%", height: 40, border: 0, borderRadius: 10, background: "#0b6ff6", color: "white", fontWeight: 900 })}>{th ? "บันทึกราคา" : "Save price"}</button></div>)}
        </div>
        <h3>{th ? `คำขอรออนุมัติ (${pending.length})` : `Pending requests (${pending.length})`}</h3>
        <div style={sx({ display: "grid", gap: 10 })}>{pending.length ? pending.map((row) => <article key={row.id} style={sx({ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 12, alignItems: "center", border: "1px solid #dbe7f4", borderRadius: 16, padding: 14 })}><div><b>{row.contract?.license_id || row.id}</b><p style={sx({ margin: "4px 0", color: "#5c6f89" })}>{row.contract?.customer_name || "-"} · {row.device?.device_code || "-"} · {row.plan_code} · {money(row.price_thb)}</p><small>{th ? "ขอเมื่อ" : "Requested"}: {when(row.requested_at)}</small></div><div style={sx({ display: "flex", gap: 8 })}><button disabled={busy} onClick={() => void approve(row)} style={sx({ padding: "10px 14px", border: 0, borderRadius: 10, background: "#16a34a", color: "white", fontWeight: 900 })}>{th ? "อนุมัติ" : "Approve"}</button><button disabled={busy} onClick={() => void reject(row)} style={sx({ padding: "10px 14px", border: "1px solid #fecaca", borderRadius: 10, background: "#fff1f2", color: "#b91c1c", fontWeight: 900 })}>{th ? "ปฏิเสธ" : "Reject"}</button></div></article>) : <div style={sx({ padding: 24, border: "1px dashed #cbd9e8", borderRadius: 14, color: "#789" })}>{th ? "ยังไม่มีคำขอซื้อ Cloud" : "No pending cloud requests"}</div>}</div>
        <h3>{th ? `Cloud ที่ใช้งาน/รอต่ออายุ (${active.length})` : `Active / renewal review (${active.length})`}</h3>
        <div style={sx({ display: "grid", gap: 10 })}>{active.map((row) => <article key={row.id} style={sx({ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 12, alignItems: "center", border: "1px solid #dbe7f4", borderRadius: 16, padding: 14 })}><div><b>{row.cloud_code}</b><p style={sx({ margin: "4px 0", color: "#5c6f89" })}>{row.contract?.license_id || "-"} · {row.contract?.customer_name || "-"} · {row.device?.device_code || "-"}</p><small>{row.status} · {th ? "หมดอายุ" : "Expires"}: {when(row.expires_at)} · {th ? "Backup ล่าสุด" : "Last backup"}: {when(row.last_backup_at)}</small></div><div style={sx({ display: "flex", gap: 8 })}><button disabled={busy || row.status !== "expired_pending"} onClick={() => void renew(row)} style={sx({ padding: "10px 14px", border: 0, borderRadius: 10, background: row.status === "expired_pending" ? "#0b6ff6" : "#cbd5e1", color: "white", fontWeight: 900 })}>{th ? "ต่อ Cloud" : "Renew"}</button><button disabled={busy} onClick={() => cancelCloud(row)} style={sx({ padding: "10px 14px", border: "1px solid #fecaca", borderRadius: 10, background: "#fff1f2", color: "#b91c1c", fontWeight: 900 })}>{th ? "ยกเลิก Cloud" : "Cancel"}</button></div></article>))}</div>
      </section>
    </div> : null}
  </>;
}
