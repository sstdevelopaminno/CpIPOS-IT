...
function initials(name: string) {
  ...
  return trimmed ? trimmed.slice(0, 2).toUpperCase() : "O";
}
...
export function TenantPrimaryOwnerCard({ tenantId }: { tenantId: string }) {
  ...
  const pinCanSave = pinIsValid && pinMatches;
  ...
  const savePin = async () => {
    ...
    try {
      ...
      const payload = await parse<OwnerResponse>(response);
      const wasConfigured = owner.pin_configured;
      setOwner(payload.primary_owner);
      setPinModalOpen(false);
      clearPinDraft();
      setSuccess(wasConfigured ? "เปลี่ยนรหัส Owner/PIN เรียบร้อยแล้ว" : "ตั้งรหัส Owner/PIN เรียบร้อยแล้ว");
    } catch (saveError) {
    ...
    return (
      <section className={styles.compactCard} data-primary-owner-card>
        ...
        <div className={styles.summary}>
          ...
          <strong>กำลังโหลด Owner…</strong>
          <small>ตรวจสอบรหัสพนักงาน POS, สิทธิ์ และสถานะ PIN</small>
        </div>
      </section>
    );
  }

  const editorDialog = editorOpen && owner && typeof document !== "undefined" ? createPortal(
    <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) closeEditor(); }}>
      <section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="primary-owner-editor-title">
        <header className={styles.modalHeader}>
          <div className={styles.modalIdentity}>
            ...
            <div>
              ...
              <p>เชื่อมรหัสพนักงาน POS, บัญชี Owner และ PIN เข้ากับสิทธิ์ของสาขาเดียวกัน</p>
            </div>
          </div>
          ...
          <section className={styles.editorSection}>
            ...
            <div className={styles.formGrid}>
              <label className={styles.fullWidth}>
                ...
                />
                <small>ใช้ระบุ USER ในหน้า POS ก่อน จากนั้น Owner/Manager ที่ตั้ง PIN จะยืนยัน PIN เป็นขั้นตอนถัดไป</small>
              </label>
          ...
          <section className={`${styles.editorSection} ${styles.securitySection}`}>
            <div className={styles.securityHeader}>
              <div>
                ...
                <p>PIN เป็นคนละค่ากับรหัสพนักงาน และใช้ยืนยันสิทธิ์ Owner/Manager หลังจากระบบพบ USER แล้ว</p>
              </div>
              <div className={styles.securityActions}>
                ...
                <button type="button" onClick={() => { setError(null); setSuccess(null); clearPinDraft(); setPinModalOpen(true); }}>{owner.pin_configured ? "เปลี่ยนรหัส" : "ตั้งรหัส"}</button>
              </div>
            </div>
          </section>
        ...
        <footer className={styles.modalFooter}>
          ...
          <button type="button" className={styles.saveButton} onClick={() => void saveProfile()} disabled={saving || pinSaving || !profileCanSave}>{saving ? "กำลังบันทึก…" : "บันทึก Owner / POS Login"}</button>
        </footer>
      </section>
    </div>,
    document.body
  ...
  const pinDialog = editorOpen && owner && pinModalOpen && typeof document !== "undefined" ? createPortal(
      ...
      <section className={styles.pinModal} role="dialog" aria-modal="true" aria-labelledby="owner-pin-dialog-title">
        <header className={styles.pinModalHeader}>
          <div>
            ...
            <p>หลังบันทึก POS จะใช้ PIN ใหม่นี้ในการยืนยัน Owner/Manager ของ USER คนนี้</p>
          </div>
    ...
    <>
      {owner ? (
        <button type="button" className={styles.compactCard} data-primary-owner-card onClick={() => { setError(null); setSuccess(null); clearPinDraft(); setEditorOpen(true); }}>
          ...
          <div className={styles.cardBottom}><span>Owner ตั้งแต่ {ownerSince}</span><strong>จัดการ Owner →</strong></div>
        </button>
      ) : (
        <section className={styles.compactCard} data-primary-owner-card>
